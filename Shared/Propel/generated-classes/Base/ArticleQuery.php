 ...
 * @method     ChildArticleQuery orderByArticleTypeId($order = Criteria::ASC) Order by the article_type_id column
 *
 ...
 * @method     ChildArticleQuery groupByArticleTypeId() Group by the article_type_id column
 *
 ...
 *
 * @method     ChildArticleQuery leftJoinArticleType($relationAlias = null) Adds a LEFT JOIN clause to the query using the ArticleType relation
 * @method     ChildArticleQuery rightJoinArticleType($relationAlias = null) Adds a RIGHT JOIN clause to the query using the ArticleType relation
 * @method     ChildArticleQuery innerJoinArticleType($relationAlias = null) Adds a INNER JOIN clause to the query using the ArticleType relation
 *
 * @method     ChildArticleQuery joinWithArticleType($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the ArticleType relation
 *
 * @method     ChildArticleQuery leftJoinWithArticleType() Adds a LEFT JOIN clause and with to the query using the ArticleType relation
 * @method     ChildArticleQuery rightJoinWithArticleType() Adds a RIGHT JOIN clause and with to the query using the ArticleType relation
 * @method     ChildArticleQuery innerJoinWithArticleType() Adds a INNER JOIN clause and with to the query using the ArticleType relation
 *
 ...
 *
 * @method     \ArticleTypeQuery|\ShapeQuery|\ArticleHasIngredientQuery|\OrderHasArticlesQuery|\PackageHasArticlesQuery|\RatingQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 *
 * @method     ChildArticle findOneByArticleId(string $article_id) Return the first ChildArticle filtered by the article_id column
 ...
 * @method     ChildArticle findOneByShapeId(string $shape_id) Return the first ChildArticle filtered by the shape_id column
 * @method     ChildArticle findOneByArticleTypeId(string $article_type_id) Return the first ChildArticle filtered by the article_type_id column *
 ...
 * @method     ChildArticle requireOneByShapeId(string $shape_id) Return the first ChildArticle filtered by the shape_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildArticle requireOneByArticleTypeId(string $article_type_id) Return the first ChildArticle filtered by the article_type_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildArticle[]|ObjectCollection findByShapeId(string $shape_id) Return ChildArticle objects filtered by the shape_id column
 * @method     ChildArticle[]|ObjectCollection findByArticleTypeId(string $article_type_id) Return ChildArticle objects filtered by the article_type_id column
...
abstract class ArticleQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT article_id, description, price, creation, visible, shape_id, article_type_id FROM article WHERE article_id = :p0';
        try {
    ...
    public function filterByArticleTypeId($articleTypeId = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(ArticleTableMap::COL_ARTICLE_TYPE_ID, $articleTypeId, $comparison);
    }

    /**
     * Filter the query by a related \ArticleType object
     *
     * @param \ArticleType|ObjectCollection $articleType The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
     ...
     */
    public function filterByArticleType($articleType, $comparison = null)
    {
        if ($articleType instanceof \ArticleType) {
            return $this
                ->addUsingAlias(ArticleTableMap::COL_ARTICLE_TYPE_ID, $articleType->getArticleTypeId(), $comparison);
        } elseif ($articleType instanceof ObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }

            return $this
                ->addUsingAlias(ArticleTableMap::COL_ARTICLE_TYPE_ID, $articleType->toKeyValue('PrimaryKey', 'ArticleTypeId'), $comparison);
        } else {
            throw new PropelException('filterByArticleType() only accepts arguments of type \ArticleType or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the ArticleType relation
     *
     ...
     * @return $this|ChildArticleQuery The current query, for fluid interface
     */
    public function joinArticleType($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('ArticleType');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'ArticleType');
        }
    ...
    /**
     * Use the ArticleType relation ArticleType object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \ArticleTypeQuery A secondary query class using the current class as primary query
     */
    public function useArticleTypeQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinArticleType($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'ArticleType', '\ArticleTypeQuery');
    }
