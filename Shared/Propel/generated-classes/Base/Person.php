...
abstract class Person implements ActiveRecordInterface
{
    ...
    /**
     * @var        ObjectCollection|ChildBusiness customer[] Collection to store aggregation of ChildBusiness customer objects.
     */
    protected $collBusiness customers;
    protected $collBusiness customersPartial;
     ...
     * @var ObjectCollection|ChildBusiness customer[]
     */
    protected $business customersScheduledForDeletion = null;
    ...
    public function setE-mail($v)
    {
        ...
        if ($this->e-mail !== $v) {
            $this->e-mail = $v;
            $this->modifiedColumns[PersonTableMap::COL_E-MAIL] = true;
        }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->aCity = null;
            $this->aType = null;
            $this->collBusiness customers = null;
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->business customersScheduledForDeletion !== null) {
                if (!$this->business customersScheduledForDeletion->isEmpty()) {
                    \Business customerQuery::create()
                        ->filterByPrimaryKeys($this->business customersScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->business customersScheduledForDeletion = null;
                }
            }

            if ($this->collBusiness customers !== null) {
                foreach ($this->collBusiness customers as $referrerFK) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collBusiness customers) {
                ...
                $result[$key] = $this->collBusiness customers->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getBusiness customers() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addBusiness customer($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ($this->aType === null && (($this->type_id !== "" && $this->type_id !== null))) {
    ...
    {
        if ('Business customer' == $relationName) {
            $this->initBusiness customers();
            return;
        }
    ...
    /**
     * Clears out the collBusiness customers collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return void
     * @see        addBusiness customers()
     */
    public function clearBusiness customers()
    {
        $this->collBusiness customers = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collBusiness customers collection loaded partially.
     */
    public function resetPartialBusiness customers($v = true)
    {
        $this->collBusiness customersPartial = $v;
    }

    /**
     * Initializes the collBusiness customers collection.
     *
     * By default this just sets the collBusiness customers collection to an empty array (like clearcollBusiness customers());
     ...
     * @return void
     */
    public function initBusiness customers($overrideExisting = true)
    {
        if (null !== $this->collBusiness customers && !$overrideExisting) {
            return;
        }

        $collectionClassName = Business customerTableMap::getTableMap()->getCollectionClassName();

        $this->collBusiness customers = new $collectionClassName;
        $this->collBusiness customers->setModel('\Business customer');
    }

    /**
     * Gets an array of ChildBusiness customer objects which contain a foreign key that references this object.
     *
     ...
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildBusiness customer[] List of ChildBusiness customer objects
     * @throws PropelException
     */
    public function getBusiness customers(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collBusiness customersPartial && !$this->isNew();
        if (null === $this->collBusiness customers || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collBusiness customers) {
                // return empty collection
                $this->initBusiness customers();
            } else {
                $collBusiness customers = ChildBusiness customerQuery::create(null, $criteria)
                    ->filterByPerson($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collBusiness customersPartial && count($collBusiness customers)) {
                        $this->initBusiness customers(false);

                        foreach ($collBusiness customers as $obj) {
                            if (false == $this->collBusiness customers->contains($obj)) {
                                $this->collBusiness customers->append($obj);
                            }
                        }

                        $this->collBusiness customersPartial = true;
                    }

                    return $collBusiness customers;
                }

                if ($partial && $this->collBusiness customers) {
                    foreach ($this->collBusiness customers as $obj) {
                        if ($obj->isNew()) {
                            $collBusiness customers[] = $obj;
                        }
                    }
                }

                $this->collBusiness customers = $collBusiness customers;
                $this->collBusiness customersPartial = false;
            }
        }

        return $this->collBusiness customers;
    }

    /**
     * Sets a collection of ChildBusiness customer objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $business customers A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildPerson The current object (for fluent API support)
     */
    public function setBusiness customers(Collection $business customers, ConnectionInterface $con = null)
    {
        /** @var ChildBusiness customer[] $business customersToDelete */
        $business customersToDelete = $this->getBusiness customers(new Criteria(), $con)->diff($business customers);


        $this->business customersScheduledForDeletion = $business customersToDelete;

        foreach ($business customersToDelete as $business customerRemoved) {
            $business customerRemoved->setPerson(null);
        }

        $this->collBusiness customers = null;
        foreach ($business customers as $business customer) {
            $this->addBusiness customer($business customer);
        }

        $this->collBusiness customers = $business customers;
        $this->collBusiness customersPartial = false;
    ...
    /**
     * Returns the number of related Business customer objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related Business customer objects.
     * @throws PropelException
     */
    public function countBusiness customers(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collBusiness customersPartial && !$this->isNew();
        if (null === $this->collBusiness customers || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collBusiness customers) {
            ...
            if ($partial && !$criteria) {
                return count($this->getBusiness customers());
            }

            $query = ChildBusiness customerQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collBusiness customers);
    }

    /**
     * Method called to associate a ChildBusiness customer object to this object
     * through the ChildBusiness customer foreign key attribute.
     *
     * @param  ChildBusiness customer $l ChildBusiness customer
     * @return $this|\Person The current object (for fluent API support)
     */
    public function addBusiness customer(ChildBusiness customer $l)
    {
        if ($this->collBusiness customers === null) {
            $this->initBusiness customers();
            $this->collBusiness customersPartial = true;
        ...
        if (!$this->collBusiness customers->contains($l)) {
            $this->doAddBusiness customer($l);

            if ($this->business customersScheduledForDeletion and $this->business customersScheduledForDeletion->contains($l)) {
                $this->business customersScheduledForDeletion->remove($this->business customersScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildBusiness customer $business customer The ChildBusiness customer object to add.
     */
    protected function doAddBusiness customer(ChildBusiness customer $business customer)
    {
        $this->collBusiness customers[]= $business customer;
        $business customer->setPerson($this);
    }

    /**
     * @param  ChildBusiness customer $business customer The ChildBusiness customer object to remove.
     * @return $this|ChildPerson The current object (for fluent API support)
     */
    public function removeBusiness customer(ChildBusiness customer $business customer)
    {
        if ($this->getBusiness customers()->contains($business customer)) {
            $pos = $this->collBusiness customers->search($business customer);
            $this->collBusiness customers->remove($pos);
            if (null === $this->business customersScheduledForDeletion) {
                $this->business customersScheduledForDeletion = clone $this->collBusiness customers;
                $this->business customersScheduledForDeletion->clear();
            }
            $this->business customersScheduledForDeletion[]= clone $business customer;
            $business customer->setPerson(null);
        }
    ...
    {
        if ($deep) {
            if ($this->collBusiness customers) {
                foreach ($this->collBusiness customers as $o) {
