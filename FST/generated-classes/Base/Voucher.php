...
/**
 * Base class that represents a row from the 'voucher' table.
 *
 *
 *
...
abstract class Voucher implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\VoucherTableMap';
    ...
    /**
     * Initializes internal state of Base\Voucher object.
     */
     ...
     *
     * @return $this|Voucher The current object, for fluid interface
     */
     ...
     * @return $this|\Voucher The current object (for fluent API support)
     */
    public function setVoucherId($v)
    {
        ...
        if ($this->voucher_id !== $v) {
            $this->voucher_id = $v;
            $this->modifiedColumns[VoucherTableMap::COL_VOUCHER_ID] = true;
        }
    ...
    public function setAmount($v)
    {
        ...
        if ($this->amount !== $v) {
            $this->amount = $v;
            $this->modifiedColumns[VoucherTableMap::COL_AMOUNT] = true;
        }
    ...
    public function setUsed($v)
    {
        ...
        if ($this->used !== $v) {
            $this->used = $v;
            $this->modifiedColumns[VoucherTableMap::COL_USED] = true;
        }
    ...
    public function setDate($v)
    {
        ...
        if ($this->date !== null || $dt !== null) {
            if ($this->date === null || $dt === null || $dt->format("Y-m-d") !== $this->date->format("Y-m-d")) {
                ...
                $this->modifiedColumns[VoucherTableMap::COL_DATE] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : VoucherTableMap::translateFieldName('VoucherId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->voucher_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : VoucherTableMap::translateFieldName('Amount', TableMap::TYPE_PHPNAME, $indexType)];
            $this->amount = (null !== $col) ? (double) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : VoucherTableMap::translateFieldName('Code', TableMap::TYPE_PHPNAME, $indexType)];
            $this->code = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : VoucherTableMap::translateFieldName('Used', TableMap::TYPE_PHPNAME, $indexType)];
            $this->used = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : VoucherTableMap::translateFieldName('Date', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            ...
            return $startcol + 5; // 5 = VoucherTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Voucher'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(VoucherTableMap::COL_VOUCHER_ID)) {
            $modifiedColumns[':p' . $index++]  = 'voucher_id';
        }
        if ($this->isColumnModified(VoucherTableMap::COL_AMOUNT)) {
            $modifiedColumns[':p' . $index++]  = 'amount';
        }
        if ($this->isColumnModified(VoucherTableMap::COL_CODE)) {
            $modifiedColumns[':p' . $index++]  = 'code';
        }
        if ($this->isColumnModified(VoucherTableMap::COL_USED)) {
            $modifiedColumns[':p' . $index++]  = 'used';
        }
        if ($this->isColumnModified(VoucherTableMap::COL_DATE)) {
            $modifiedColumns[':p' . $index++]  = 'date';
        }

        $sql = sprintf(
            'INSERT INTO voucher (%s) VALUES (%s)',
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(VoucherTableMap::COL_VOUCHER_ID)) {
            $criteria->add(VoucherTableMap::COL_VOUCHER_ID, $this->voucher_id);
        }
        if ($this->isColumnModified(VoucherTableMap::COL_AMOUNT)) {
            $criteria->add(VoucherTableMap::COL_AMOUNT, $this->amount);
        }
        if ($this->isColumnModified(VoucherTableMap::COL_CODE)) {
            $criteria->add(VoucherTableMap::COL_CODE, $this->code);
        }
        if ($this->isColumnModified(VoucherTableMap::COL_USED)) {
            $criteria->add(VoucherTableMap::COL_USED, $this->used);
        }
        if ($this->isColumnModified(VoucherTableMap::COL_DATE)) {
            $criteria->add(VoucherTableMap::COL_DATE, $this->date);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Voucher (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Voucher Clone of current object.
     * @throws PropelException
     */
     ...
     * @return ObjectCollection|ChildOrder[] List of ChildOrder objects
     */
    public function getOrdersJoinPerson(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildOrderQuery::create(null, $criteria);
