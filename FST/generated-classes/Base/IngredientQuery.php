 ...
 *
 * @method     ChildIngredientQuery leftJoinIngredientHasAllergy($relationAlias = null) Adds a LEFT JOIN clause to the query using the IngredientHasAllergy relation
 * @method     ChildIngredientQuery rightJoinIngredientHasAllergy($relationAlias = null) Adds a RIGHT JOIN clause to the query using the IngredientHasAllergy relation
 * @method     ChildIngredientQuery innerJoinIngredientHasAllergy($relationAlias = null) Adds a INNER JOIN clause to the query using the IngredientHasAllergy relation
 *
 * @method     ChildIngredientQuery joinWithIngredientHasAllergy($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the IngredientHasAllergy relation
 *
 * @method     ChildIngredientQuery leftJoinWithIngredientHasAllergy() Adds a LEFT JOIN clause and with to the query using the IngredientHasAllergy relation
 * @method     ChildIngredientQuery rightJoinWithIngredientHasAllergy() Adds a RIGHT JOIN clause and with to the query using the IngredientHasAllergy relation
 * @method     ChildIngredientQuery innerJoinWithIngredientHasAllergy() Adds a INNER JOIN clause and with to the query using the IngredientHasAllergy relation
 *
 * @method     \ArticleHasIngredientQuery|\IngredientHasAllergyQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 *
 * @method     ChildIngredient findOneByIngredientId(int $ingredient_id) Return the first ChildIngredient filtered by the ingredient_id column
 ...
 *
 * @method     ChildIngredient requireOneByIngredientId(int $ingredient_id) Return the first ChildIngredient filtered by the ingredient_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildIngredient[]|ObjectCollection findByIngredientId(int $ingredient_id) Return ChildIngredient objects filtered by the ingredient_id column
...
abstract class IngredientQuery extends ModelCriteria
{
    ...
    public function filterByIngredientId($ingredientId = null, $comparison = null)
    {
        if (is_array($ingredientId)) {
            ...
            if (isset($ingredientId['min'])) {
                $this->addUsingAlias(IngredientTableMap::COL_INGREDIENT_ID, $ingredientId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($ingredientId['max'])) {
                $this->addUsingAlias(IngredientTableMap::COL_INGREDIENT_ID, $ingredientId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    public function filterByIngredientHasAllergy($ingredientHasAllergy, $comparison = null)
    {
        if ($ingredientHasAllergy instanceof \IngredientHasAllergy) {
            return $this
                ->addUsingAlias(IngredientTableMap::COL_INGREDIENT_ID, $ingredientHasAllergy->getIngredientId(), $comparison);
