 ...
 *
 * @method     ChildSpecialOffer findOneBySpecialOfferId(int $special_offer_id) Return the first ChildSpecialOffer filtered by the special_offer_id column
 ...
 *
 * @method     ChildSpecialOffer requireOneBySpecialOfferId(int $special_offer_id) Return the first ChildSpecialOffer filtered by the special_offer_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildSpecialOffer[]|ObjectCollection findBySpecialOfferId(int $special_offer_id) Return ChildSpecialOffer objects filtered by the special_offer_id column
...
abstract class SpecialOfferQuery extends ModelCriteria
{
     ...
     * Example usage:
     * <code>
     * $query->filterBySpecialOfferId(1234); // WHERE special_offer_id = 1234
     * $query->filterBySpecialOfferId(array(12, 34)); // WHERE special_offer_id IN (12, 34)
     * $query->filterBySpecialOfferId(array('min' => 12)); // WHERE special_offer_id > 12
     * </code>
     *
     * @param     mixed $specialOfferId The value to use as filter.
    ...
    public function filterBySpecialOfferId($specialOfferId = null, $comparison = null)
    {
        if (is_array($specialOfferId)) {
            $useMinMax = false;
            if (isset($specialOfferId['min'])) {
                $this->addUsingAlias(SpecialOfferTableMap::COL_SPECIAL_OFFER_ID, $specialOfferId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($specialOfferId['max'])) {
                $this->addUsingAlias(SpecialOfferTableMap::COL_SPECIAL_OFFER_ID, $specialOfferId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
