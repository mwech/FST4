 ...
 *
 * @method     ChildShape findOneByShapeId(int $shape_id) Return the first ChildShape filtered by the shape_id column
 ...
 *
 * @method     ChildShape requireOneByShapeId(int $shape_id) Return the first ChildShape filtered by the shape_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildShape[]|ObjectCollection findByShapeId(int $shape_id) Return ChildShape objects filtered by the shape_id column
    ...
    public function filterByShapeId($shapeId = null, $comparison = null)
    {
        if (is_array($shapeId)) {
            ...
            if (isset($shapeId['min'])) {
                $this->addUsingAlias(ShapeTableMap::COL_SHAPE_ID, $shapeId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($shapeId['max'])) {
                $this->addUsingAlias(ShapeTableMap::COL_SHAPE_ID, $shapeId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
