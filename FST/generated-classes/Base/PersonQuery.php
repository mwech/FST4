 ...
 *
 * @method     ChildPersonQuery leftJoinBusinessCustomer($relationAlias = null) Adds a LEFT JOIN clause to the query using the BusinessCustomer relation
 * @method     ChildPersonQuery rightJoinBusinessCustomer($relationAlias = null) Adds a RIGHT JOIN clause to the query using the BusinessCustomer relation
 * @method     ChildPersonQuery innerJoinBusinessCustomer($relationAlias = null) Adds a INNER JOIN clause to the query using the BusinessCustomer relation
 *
 * @method     ChildPersonQuery joinWithBusinessCustomer($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the BusinessCustomer relation
 *
 * @method     ChildPersonQuery leftJoinWithBusinessCustomer() Adds a LEFT JOIN clause and with to the query using the BusinessCustomer relation
 * @method     ChildPersonQuery rightJoinWithBusinessCustomer() Adds a RIGHT JOIN clause and with to the query using the BusinessCustomer relation
 * @method     ChildPersonQuery innerJoinWithBusinessCustomer() Adds a INNER JOIN clause and with to the query using the BusinessCustomer relation
 *
 ...
 *
 * @method     \CityQuery|\TypeQuery|\BusinessCustomerQuery|\OrderQuery|\RatingQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildPerson findOneByTypeId(int $type_id) Return the first ChildPerson filtered by the type_id column *
 ...
 *
 * @method     ChildPerson requireOneByPersonId(int $person_id) Return the first ChildPerson filtered by the person_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildPerson requireOneByTypeId(int $type_id) Return the first ChildPerson filtered by the type_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildPerson[]|ObjectCollection findByTypeId(int $type_id) Return ChildPerson objects filtered by the type_id column
...
abstract class PersonQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT person_id, firstname, lastname, email, password, birthdate, street, country, zip_code, type_id FROM person WHERE person_id = :p0';
        try {
    ...
    public function filterByTypeId($typeId = null, $comparison = null)
    {
        if (is_array($typeId)) {
            $useMinMax = false;
            if (isset($typeId['min'])) {
                $this->addUsingAlias(PersonTableMap::COL_TYPE_ID, $typeId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($typeId['max'])) {
                $this->addUsingAlias(PersonTableMap::COL_TYPE_ID, $typeId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    /**
     * Filter the query by a related \BusinessCustomer object
     *
     * @param \BusinessCustomer|ObjectCollection $businessCustomer the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildPersonQuery The current query, for fluid interface
     */
    public function filterByBusinessCustomer($businessCustomer, $comparison = null)
    {
        if ($businessCustomer instanceof \BusinessCustomer) {
            return $this
                ->addUsingAlias(PersonTableMap::COL_PERSON_ID, $businessCustomer->getPersonId(), $comparison);
        } elseif ($businessCustomer instanceof ObjectCollection) {
            return $this
                ->useBusinessCustomerQuery()
                ->filterByPrimaryKeys($businessCustomer->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByBusinessCustomer() only accepts arguments of type \BusinessCustomer or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the BusinessCustomer relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return $this|ChildPersonQuery The current query, for fluid interface
     */
    public function joinBusinessCustomer($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('BusinessCustomer');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'BusinessCustomer');
        }
    ...
    /**
     * Use the BusinessCustomer relation BusinessCustomer object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \BusinessCustomerQuery A secondary query class using the current class as primary query
     */
    public function useBusinessCustomerQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinBusinessCustomer($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'BusinessCustomer', '\BusinessCustomerQuery');
    }
